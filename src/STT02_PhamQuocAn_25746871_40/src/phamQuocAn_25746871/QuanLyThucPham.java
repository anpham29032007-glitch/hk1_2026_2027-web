...
public class QuanLyThucPham {
    ...
    public QuanLyThucPham() {
        this.khang = KICH_THUOC_BAN_DAU;
        this.danhSach = new HangThucPham[khang];
        this.soLuong = 0;
    }

    // ------------ Mở rộng mảng -------------
    private void moRongMang() {
        int kichThuocMoi = khang * 2;
        ...
        danhSach = mangMoi;
        khang = kichThuocMoi;
        System.out.println("Đã mở rộng mảng lên " + khang + " phần tử.");
    }
    ...
    public void sapXepTheoNgaySX(boolean tangDan) {
        for (int i = 0; i < soLuong - 1; i++) {
            for (int j = i + 1; j < soLuong; j++) {
    ...
    public QuanLyThucPham timGanDung(String tukhoa) {
        QuanLyThucPham ketQua = new QuanLyThucPham();
        String tk = tukhoa.toLowerCase();
        for (int i = 0; i < soLuong; i++) {
